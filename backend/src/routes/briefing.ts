...
import { get_today_meetings } from '../agent/tools';
...
async function generateBriefingContent(userId: number): Promise<string> {
  const meetings = await get_today_meetings(userId);

  const notifs = await pool.query(
    `SELECT message, created_at FROM ${schemaName}.notifications
     WHERE user_id = $1 AND is_read = FALSE ORDER BY created_at DESC LIMIT 5`,
    [userId]
  );
...
${meetings.length === 0
  ...
  : meetings.map((m: any) =>
    `- ${m.title} at ${new Date(m.start_time).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit' })}${m.location ? ` @ ${m.location}` : ''} [${m.status}]`
  ).join('\n')
}
...
${notifs.rows.length === 0
  ? '(None)'
  : notifs.rows.map((n: any) => `- ${n.message}`).join('\n')
}
...
router.get('/today', async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existing = await pool.query(
      `SELECT brief_date, content, model, created_at
    ...
    const saved = await pool.query(
      `INSERT INTO ${schemaName}.briefings (user_id, brief_date, content, model)
       VALUES ($1, CURRENT_DATE, $2, $3)
       ON CONFLICT (user_id, brief_date) DO UPDATE SET content = $2, model = $3, created_at = NOW()
       RETURNING brief_date, content, model, created_at`,
      [req.userId, content, BRIEFING_MODEL]
    );
