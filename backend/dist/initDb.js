...
async function initDb() {
    ...
    `);
        ...
        await client.query(`
      CREATE TABLE IF NOT EXISTS ${db_1.schemaName}.contacts (
        id          SERIAL PRIMARY KEY,
        user_id     INTEGER NOT NULL REFERENCES ${db_1.schemaName}.users(id) ON DELETE CASCADE,
    ...
    `);
        ...
        if (existing.rows.length === 0) {
            ...
            const userInsert = await client.query(`INSERT INTO ${db_1.schemaName}.users (full_name, username, password)
         VALUES ($1, $2, $3) RETURNING id`, ['Principal User', defaultEmail, hashed]);
        ...
        for (const dc of defaultContacts) {
            await client.query(`INSERT INTO ${db_1.schemaName}.contacts (user_id, name, email, designation, department)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (user_id, email) DO NOTHING`, [userId, dc.name, dc.email, dc.designation, dc.department]);
        }
